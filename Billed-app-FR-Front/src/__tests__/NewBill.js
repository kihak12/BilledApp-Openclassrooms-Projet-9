...
describe("Given I am connected as an employee", () => {
  ...
  describe("When I select an image in a correct format",  () => {
    test("Then the input file should display the file name", async() => {
      ...
      await waitFor(() => screen.getByTestId('invalid-file'))
      const invalidFileMessage = screen.getByTestId('invalid-file')

      expect(invalidFileMessage).toHaveStyle('display: none');
  ...
  describe("When I select a file with an wrong format", () => {
    test("Then the input file should not display the file name and an error appear", async () => {
      const newBillUI = NewBillUI();
      document.body.innerHTML = newBillUI;
      const onNavigate = (pathname) => {
        document.body.innerHTML = ROUTES({ pathname });
      ...
      fireEvent.change(input, {
        target: {
          files: [new File(['wrong.mp4'], 'wrong.mp4', {
            type: 'video/mp4'
          })],
        }
      })
      ...
      expect(invalidFileMessage).toHaveStyle('display: block');
      expect(handleChangeFile).toHaveBeenCalled()
...
describe("Given I am a user connected as Employee", () => {
  ...
  describe("When I add a new bill", () => {
    ...
    test("Then it fails with a 404 message error", async() => {
      const newBillUI = BillsUI({ error: 'Erreur 404' })
      document.body.innerHTML = newBillUI;
      const message = await screen.getByText(/Erreur 404/);
      expect(message).toBeTruthy();
    })
    test("Then it fails with a 500 message error", async() => {
      const newBillUI = BillsUI({ error: 'Erreur 500' })
      document.body.innerHTML = newBillUI;
